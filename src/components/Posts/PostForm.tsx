...
const PostForm: FC<PostFormProps> = ({ door }) => {
  ...
  if (isSubmitted) {
    return (
      <div className="bg-purple-500 rounded-md px-8 py-4 w-96 space-y-4 grid place-content-center">
  ...
  return (
    <div className="bg-purple-500 w-full rounded-md px-4 pt-4 pb-2 flex flex-col items-end">
      {preview && (
        <PostPreview
          html={previewHtml}
          isLoading={previewLoading}
          className="w-full min-h-[5rem] pb-2 rounded-b-none border-b-2 border-white"
        />
      )}
      ...
      <TextareaAutosize
        className={clsx(
          "block w-full h-20 p-0 pb-2 outline-none bg-transparent border-b-2 border-white",
          preview && "hidden"
        )}
        ref={inputRef}
        onChange={setDirty}
