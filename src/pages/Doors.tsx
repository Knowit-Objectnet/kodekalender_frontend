...
const Doors = () => {
      ...
      map(zip(doorsState, doorElementStyles, doorElementRefs), ([state, styles, ref], i) =>
        state === "closed" ? null : (
          <Link
            ...
            className={cl(
              "fixed",
              getAnchorVar("debug") && "bg-red-700/40 ring-4 ring-inset ring-red-700"
            )}
            onMouseOver={() => {
              prefetchLikes()
              prefetchPosts(i + 1)
  ...
  useEffect(() => {
    ...
    const linkIndex =
      currentDate.getHours() >= 4 ? currentDate.getDate() : currentDate.getDate() - 1
    ...
    doorsContainerRef.current.scrollTo({
      behavior: "instant",
      left: boundingRect.left - boundingRect.width / 2 - doorsContainerWidth / 2
    })
  }, [])
